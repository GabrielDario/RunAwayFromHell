...
const checkFloor = {
  3 : {
    1: (carry) => { carry.push("Não tem nada nesa sala!") },
    2: (carry) => { carry.push("Não tem nada nesa sala!") },
...
const executeAction = (direct, room, floor) => {
  ...
  if (actions[floor] && actions[floor][direct]) {
    (actions[floor][direct][room])(carry);
  }
  return carry
};

const checking = (floor,room) => {
  console.log('cheking...')
  let carry = []
  if (checkFloor[floor] && checkFloor[floor][room]) {
    (checkFloor[floor][room])(carry);
  }
  return carry
}
